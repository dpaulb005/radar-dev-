...
LAYOUT = BOARD / "layout"
ASSY   = BOARD / "assembly"
OUT    = TOOLS                      # layout.json stays beside the generator
NETS   = BOARD / "schematics" / "radar_breadboard_nets.json"
...
def expected_nets():
    nets = json.loads(NETS.read_text(encoding="utf-8"))
...
# label placement
#
# Labels used to be emitted at a fixed offset from their part, which is why R1
# sat on top of C20, C5 on R10 and J10 on R19: sixty parts on a 165 mm board
# leave no room for a fixed rule. They are collected instead, and each is placed
# in the first candidate slot that collides with nothing already placed and no
# part body. A label that has to move far enough to lose its part gets a leader.
# ----------------------------------------------------------------------
CHARW = 0.60                       # monospace advance, in em
LEAD_AT = 1.9                      # mm of travel before a leader line is drawn


def txt_w(s, fs, S):               # rendered width of a string, in board mm
    return CHARW * fs * len(s) / S


def hits(a, b, pad=0.35):      # 0.35 mm of air; 0.55 makes the 5 V block unplaceable
    return not (a[2] <= b[0] - pad or a[0] >= b[2] + pad or
                a[3] <= b[1] - pad or a[1] >= b[3] + pad)


def mklabel(ax, ay, lines, S, prefer="h", fill="#111", halo="#f4f1e8", bias=(0, 0)):
    """A label that can be set either way up, with a ladder of places to try.

    Stacked horizontally it is wide and short; rotated it is narrow and tall.
    Around the 5 V distribution block, where a bead, two capacitors and a header
    share three columns, the only thing that fits is often the other orientation
    — so both are offered and the placer takes whichever lands clear first.
    """
    wh = max(txt_w(t, f, S) for t, f, _ in lines) + 0.6
    hh = sum(f * 1.15 / S for _, f, _ in lines) + 0.3
    one = " ".join(t for t, _, _ in lines)
    fs_v = lines[0][1]
    wv = fs_v * 1.15 / S + 0.3
    hv = txt_w(one, fs_v, S) + 0.6
    # rings of offsets, nearest first; the preferred orientation gets each ring
    # before the other one does
    rings = [(0, 0)]
    for r in (2.6, 4.4, 6.2, 8.2, 10.4):
        rings += [(0, -r), (0, r), (-r, 0), (r, 0),
                  (-r * .75, -r * .75), (r * .75, -r * .75),
                  (-r * .75, r * .75), (r * .75, r * .75)]
    cand = []
    for dx, dy in rings:
        dx, dy = dx + bias[0], dy + bias[1]
        if prefer == "h":
            cand += [(dx, dy, False), (dx, dy, True)]
        else:
            cand += [(dx, dy, True), (dx, dy, False)]
    return dict(ax=ax, ay=ay, lines=lines, one=one, fill=fill, halo=halo,
                wh=wh, hh=hh, wv=wv, hv=hv, cand=cand, fs_v=fs_v)


def place_labels(o, S, labels, obstacles):
    """Choose a slot for every deferred label, then emit it.

    Bigger labels go first: they are the hardest to fit, and letting a one-line
    ref take the only gap a two-line label could have used is what produces an
    unplaceable pile. Ties break left to right so the output is stable run to
    run. Anything that has to travel more than LEAD_AT gets a leader line back
    to its part, because a label that has moved off its component is worse than
    useless if you cannot tell which one it belongs to.
    """
    taken, crowded = list(obstacles), []
    labels.sort(key=lambda L: (-max(L["wh"] * L["hh"], L["wv"] * L["hv"]), L["ax"]))
    for L in labels:
        best, best_cost = None, None
        for i, (dx, dy, rot) in enumerate(L["cand"]):
            w, h = (L["wv"], L["hv"]) if rot else (L["wh"], L["hh"])
            cx, cy = L["ax"] + dx, L["ay"] + dy
            box = (cx - w / 2, cy - h / 2, cx + w / 2, cy + h / 2)
            if box[0] < 5.0 or box[2] > 165.0 or box[1] < 0.6 or box[3] > 43.0:
                continue                                # keep it on the board
            clash = sum(1 for t in taken if hits(box, t))
            cost = clash * 100 + i
            if best_cost is None or cost < best_cost:
                best, best_cost = (cx, cy, box, rot), cost
            if clash == 0:
                break
        if best is None:
            cx, cy = L["ax"], L["ay"]
            best = (cx, cy, (cx - L["wh"] / 2, cy - L["hh"] / 2,
                             cx + L["wh"] / 2, cy + L["hh"] / 2), False)
        cx, cy, box, rot = best
        if best_cost is not None and best_cost >= 100:
            crowded.append(L["one"])          # placed, but still touching something
        taken.append(box)
        if abs(cx - L["ax"]) + abs(cy - L["ay"]) > LEAD_AT:
            o.append(f'<line x1="{L["ax"]*S}" y1="{L["ay"]*S}" x2="{cx*S}" y2="{cy*S}" '
                     f'stroke="#9a9384" stroke-width=".7" opacity=".85"/>')
        if rot:
            o.append(f'<text transform="translate({cx*S},{cy*S}) rotate(-90)" '
                     f'font-size="{L["fs_v"]}" fill="{L["fill"]}" text-anchor="middle" '
                     f'dominant-baseline="middle" font-weight="bold" stroke="{L["halo"]}" '
                     f'stroke-width="2.2" paint-order="stroke">{esc(L["one"])}</text>')
            continue
        y = cy - L["hh"] / 2
        for text, fs, fill in L["lines"]:
            y += fs * 1.15 / S
            o.append(f'<text x="{cx*S}" y="{y*S}" font-size="{fs}" fill="{fill}" '
                     f'text-anchor="middle" font-weight="bold" stroke="{L["halo"]}" '
                     f'stroke-width="2.6" paint-order="stroke">{esc(text)}</text>')
    return crowded
...
def svg(active=None, path=None):
    ...
    for p in P:
        ...
        if p["kind"] == "dip8":
            ...
            obstacles.append((x0, y - 1.27, x1, y + 1.27))
            if len(p["pins"]) >= 4:
                # emitted after every part body: R21 and R22 cross J11 on their
                # way to the rail and would otherwise paint over its name
                ontop.append(f'<text x="{(x0+x1)/2*S}" y="{(y+0.5)*S}" font-size="6.6" '
                             f'fill="#f4f1e8" text-anchor="middle" font-weight="bold" '
                             f'stroke="#222" stroke-width="2.4" paint-order="stroke">'
                             f'{esc(p["ref"] + " " + p["value"])}</text>')
            else:
                labels.append(mklabel(
                    (x0 + x1) / 2, y, [(p["ref"], 7.5, "#222")], S, prefer="h",
                    fill="#222", bias=(0, 3.2 if y > 22 else -2.4)))
        else:
            ...
            if ang == 0:
                obstacles.append((mx - bw / 2, my - bh / 2, mx + bw / 2, my + bh / 2))
                labels.append(mklabel(
                    mx, my, [(p["ref"], 7, "#111"), (p["value"], 6.5, "#333")], S,
                    prefer="h", bias=(0, -2.9)))       # above the part, as before
            else:
                obstacles.append((mx - bh / 2, my - bw / 2, mx + bh / 2, my + bw / 2))
                # a vertical part is tall enough to carry its own name, so start
                # rotated on the body and only break out if that is occupied
                labels.append(mklabel(
                    mx, my, [(p["ref"], 6.5, "#111"), (p["value"], 6.5, "#111")], S,
                    prefer="v"))
        o.append('</g>')
    o.extend(ontop)
    crowded = place_labels(o, S, labels, obstacles)
    if active is None and crowded:
        print(f"  {len(crowded)} label(s) could not be placed clear: {', '.join(crowded)}")
    legend(o, S, W_, H_, active)
    o.append('</svg>')
    (path or (LAYOUT / "breadboard.svg")).write_text("\n".join(o), encoding="utf-8")
...
def assembly_md():
    ...
    (ASSY / "ASSEMBLY.md").write_text("\n".join(L) + "\n", encoding="utf-8")


def wiring_md(exp, pin_node, find):
    L = ["# Breadboard wiring — every hole, every wire\n",
         "Generated by `tools/layout.py` and **verified against the schematic netlist**: every net is one",
    ...
    L.append("\n`python tools/layout.py` re-runs the check; edit the placement at the top of that file and it refuses to write if a net splits or shorts.")
    (LAYOUT / "WIRING.md").write_text("\n".join(L) + "\n", encoding="utf-8")
...
if __name__ == "__main__":
    ...
    (OUT / "layout.json").write_text(json.dumps(dict(parts=P, wires=W, rails=RAILS, nets=exp, group_net=groups, steps=STEPS, wire_function=WIRE_FUNCTION), indent=1), encoding="utf-8")
    if errs := check_steps():
    ...
    sd = ASSY / "steps"; sd.mkdir(parents=True, exist_ok=True)
    for st in STEPS:
