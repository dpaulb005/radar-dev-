...
Generated by `layout.py` (on the `dev` branch) from the same placement the schematic check runs
against, so these steps cannot drift from [`WIRING.md`](../layout/WIRING.md) or from the
board.
...
> **Check before moving on.** Ohm-meter, all with power off: GND to VANA, GND to V5 and GND to 3V3 must each read open (over 1 kohm). Any of them near zero is a short — find it before you connect 12 V. Then work the test blocks in `../multisim/`.

---

## When it is all in

The finished board: [`breadboard.png`](../layout/breadboard.png), every hole and net in
[`WIRING.md`](../layout/WIRING.md), and [`breadboard.html`](../layout/breadboard.html) if you want
to hover a part and see what it touches.

Simulate before you trust it — [`multisim/README.md`](../multisim/README.md) has one test block per circuit,
