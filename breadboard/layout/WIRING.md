...
Generated by `layout.py` on the [`dev` branch](https://github.com/dpaulb005/radar-dev-/tree/dev/hardware/breadboard) and **verified against the schematic netlist**: every net is one
...
The generator (`layout.py` on the `dev` branch) re-runs this check and refuses to write if a net splits or shorts.
