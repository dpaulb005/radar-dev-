# Multisim — the radar's baseband blocks, ready to open
...
B-sources, no `.include`, everything in one file.** (The ngspice decks they were
derived from live in `hardware/spice/` on the [`dev` branch](https://github.com/dpaulb005/radar-dev-/tree/dev/hardware/spice);
those contain `.control` blocks Multisim rejects, so they are not here.)
...
## Route B — Arbitrary SPICE Block, one black box on your own sheet (the `.sub` files in `blocks/`)
...
|---|---|---|
| `blocks/opamp_tl072.sub` | `TL072X inp inn vcc vee out` | generic TL072 (Aol 100 dB, GBW 3 MHz, clamps 1.5 V inside the rails). Used by the two blocks below; in Multisim you can instead build the amp from library TL072CPs — see the KiCad sheet |
| `blocks/video_amp.sub` | `VIDEO_AMP if audio vana vref gnd` | test card 1 — the whole video amplifier, full gain (R4 1k, R7 10k) |
| `blocks/vref_bias.sub` | `VREF_BIAS vana vref gnd` | test card 2 |
| `blocks/power_in.sub` | `POWER_IN vin vprot vana gnd` | test card 3 |
| `blocks/sync_div.sub` | `SYNC_DIV gpio sync sc gnd` | test card 4, includes the sound card's coupling |
| `blocks/fmcw_beat.sub` | `FMCW_BEAT tx leak_en beat gnd` | test card 6 — delay lines, gated leakage, POLY multiplier (the mixer), the amp's filters |
...
Everything to the right of the mixer's IF port — ADF4351, SPF5189Z amplifiers, splitter, mixer at 2.44 GHz, the horns. No models exist at that frequency in Multisim; those stages are measured on the bench (`docs/radar-hardware.md` on the `dev` branch, checkpoints 7–8).

## Reproduce the reference numbers
```
cd breadboard/multisim
ngspice -b import/03_power_in.cir    # the import files run as-is in ngspice too
```
The `_h_*.cir` harnesses that wrap each `.sub` with its sources are on the
[`dev` branch](https://github.com/dpaulb005/radar-dev-/tree/dev/hardware/spice/multisim).

## If Multisim refuses a file
- **"Unrecognized command" / it stops at a line** — you opened an ngspice deck
  (from the `dev` branch) rather than one of the files in `import/`. They contain `.control`.
