...
**Build it step by step:** [`hardware/breadboard/ASSEMBLY.md`](../hardware/breadboard/ASSEMBLY.md)
is the guide — 17 steps, a picture of the board at each one, and a measurement to
take before moving on. Power first, then the op-amps, then the signal left to
right. Jumpers follow a colour code (black GND, red VANA, orange V5, yellow 3V3,
green signal, violet VREF, blue SPI, brown stepper, pink SYNC) and there is no
white or grey wire in the build, because neither is readable on a cream board.

**Hole-by-hole reference:** [`hardware/breadboard/WIRING.md`](../hardware/breadboard/WIRING.md)
