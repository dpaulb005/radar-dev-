...
- **`firmware/drone_beacon/`** — runs on the drone's ESP32 (mergeable into
  your existing flight sketch). Broadcasts ESP-NOW beacons at 25 Hz; on
  S2/S3/C3/C6 chips also enables an 802.11mc FTM responder.
...
1. **Flash the drone**: open `firmware/drone_beacon/drone_beacon.ino` in
   Arduino IDE (with the arduino-esp32 core installed), flash the drone's
   ESP32. Note the **STA MAC** it prints at boot.
...
3. **Flash the hub**: `firmware/hub_node/hub_node.ino`; note the MAC it
   prints and put it into the sniffers' `HUB_MAC`. Keep every firmware on the
   same `WIFI_CHANNEL` (default 6).
...
| FTM upgrade | ~0.5–2 m | Needs S2/S3/C3/C6 chips both ends; run `rx_node_ftm` nodes on their own USB ports and add them with `--port`. |
