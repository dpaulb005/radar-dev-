...
- **Parts ordered:** everything in the bill of materials is on its way.
- **Simulation in progress:** the horn in HFSS (probe match, gain, patterns)
  and each breadboard block in Multisim. The ngspice reference numbers each
  block should reproduce are in `breadboard/multisim/README.md`.
- **Next, while the RF parts ship:** cut the three horns and wire the
  breadboard.
